...
# Usage:
# start -f txonlyburst.py -m 39gbps -t packet_len=9000 -t burst_size=10000 -t bursts=100

class STLS1(object):
    def create_stream (self, packet_len, burst_size, bursts, port_id):
        ...
        return STLStream(
                ...
                mode=STLTXMultiBurst(pkts_per_burst=burst_size, ibg=3.0 * 1000.0 * 1000.0, count=bursts))
    ...
    def get_streams (self, tunables, port_id, **kwargs):
        ...
        parser.add_argument('--burst_size', type=int, default=1000,
                            help='The many packets each burst to send')
        parser.add_argument('--bursts', type=int, default=10,
                            help='How many bursts (3 sec spacing) to send')
        args=parser.parse_args(tunables)
        return [self.create_stream(args.packet_len - 4, args.burst_size, args.bursts, port_id)]
